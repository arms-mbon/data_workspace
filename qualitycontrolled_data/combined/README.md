...
Note that some replicates of samples (biological replicates, which are indicated by adding the suffix _s# to the MaterialSampleID) and of the sequencing (technical replicates, which are indicated by adding the suffix _r# to the MaterialSampleID) took place, more so in the later years of the project. 
